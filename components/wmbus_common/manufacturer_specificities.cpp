...
mustTransformDiehlAddress(DiehlFrameInterpretation interpretation) {
...
DiehlFrameInterpretation detectDiehlFrameInterpretation(uchar c_field,
                                                        int m_field,
                                                        uchar ci_field,
                                                        int tpl_cfg) {
  if (diehl_manufacturers.find(m_field) != diehl_manufacturers.end()) {
    if (c_field == 0x44 || c_field == 0x46) // SND_NR / SND_IR
    {
      ...
      case 0x7A: // EN 13757-3 Application Layer (short tplh)
        if (((tpl_cfg >> 8) & 0x10) ==
            0x10) // Bit 12 from MMMMM bits of CFG field
        {
          return DiehlFrameInterpretation::REAL_DATA;
        }
...
void transformDiehlAddress(std::vector<uchar> &frame,
                           DiehlAddressTransformMethod transform_method) {
  if (transform_method == DiehlAddressTransformMethod::SWAPPING) {
    ...
    debug("(diehl) Pre-processing: setting device type to water meter for SAP "
          "PRIOS\n");
    frame[8] = 0x00; // version field is used by IZAR as part of meter id on 5
                     // bytes instead of 4
    frame[9] = 0x07; // water meter
  } else if (transform_method ==
             DiehlAddressTransformMethod::SAP_PRIOS_STANDARD) {
    warning("(diehl) Pre-processing: SAP PRIOS STANDARD transformation not "
            "implemented!\n"); // TODO
  }
}
...
uint32_t uint32FromBytes(const std::vector<uchar> &data, int offset,
  ...
  if (reverse)
    return ((uint32_t)data[offset + 3] << 24) |
           ...
           ((uint32_t)data[offset + 1] << 8) | (uint32_t)data[offset];
  else
...
void addDefaultManufacturerKeyIfAny(const std::vector<uchar> &frame,
  ...
  if (!meter_keys->hasConfidentialityKey() &&
    ...
    meter_keys->confidentiality_key =
        std::vector<uchar>(half.begin(), half.end());
    meter_keys->confidentiality_key.insert(
        meter_keys->confidentiality_key.end(), half.begin(), half.end());
...
void initializeDiehlDefaultKeySupport(
    const std::vector<uchar> &confidentiality_key,
...
bool decryptDielhRealData(Telegram *t, std::vector<uchar> &frame,
  ...
  for (auto &key : keys) {
    decoded_content = decodeDiehlLfsr(
        t->original.empty() ? frame : t->original, frame, key,
        DiehlLfsrCheckMethod::CHECKSUM_AND_0XEF, frame[14] & 0xEF);
  ...
  if (decoded_content.empty()) {
    if (!t->isSimulated() && t->parserWarns()) {
      // Do not warn when decryption is not needed since we are running from a
      // simulated file. Or if the telegram cannot be decrypted when listening
      // to all.
      warning("(diehl) Decoding LFSR real data failed.\n");
    } else {
      if (t->isSimulated()) {
        debug("(diehl) Decoding LFSR real data failed, probably already "
              "decoded since telegram is simulated.\n");
      } else {
...
const char *toString(DiehlAddressTransformMethod m) {
  switch (m) {
  case DiehlAddressTransformMethod::NONE:
    return "NONE";
  case DiehlAddressTransformMethod::SWAPPING:
    return "SWAPPING";
  case DiehlAddressTransformMethod::SAP_PRIOS:
    return "SAP_PRIOS";
  case DiehlAddressTransformMethod::SAP_PRIOS_STANDARD:
    return "SAP_PRIOS_STANDARD";
  }
  return "?";
}
...
void qdsExtractWalkByField(Telegram *t, Meter *driver, DVEntry &mfctEntry,
  ...
  std::string info =
      "*** " + bytes + " (" + fieldInfo->renderJson(driver, &fieldEntry) + ")";

  t->addSpecialExplanation(mfctEntry.offset + pos / 2, n / 2,
