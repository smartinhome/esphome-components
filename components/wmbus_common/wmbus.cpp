...
#define X(name, lcname, option, val)                                           \
  {LinkMode::name, #name, #lcname, #option, val},
    LIST_OF_LINK_MODES
#undef X
};
...
void LinkModeSet::unionLinkModeSet(LinkModeSet lms) { set_ |= lms.set_; }

void LinkModeSet::disjunctionLinkModeSet(LinkModeSet lms) { set_ &= lms.set_; }
...
#define X(key, code, name)                                                     \
  manufacturers_.push_back(Manufacturer(#key, code, name));
...
std::string manufacturer(int m_field) {
  for (auto &m : manufacturers_) {
    if (m.m_field == m_field)
      return m.name;
  }
  // Some weird meters send the first char in lower case aPT iTW. Fix and try
  // again.
  m_field &= 0x7fff;
  for (auto &m : manufacturers_) {
    if (m.m_field == m_field)
...
std::string mediaType(int a_field_device_type, int m_field) {
  ...
  case 0x1B:
    return "Room sensor (eg temperature or humidity)";
  case 0x1C:
    return "Gas detector";
  case 0x1D:
  ...
  if (m_field == MANUFACTURER_TCH) {
    ...
    case 0x62:
      return "Warm water"; // MKRadio3/MKRadio4
    case 0x72:
      return "Cold water"; // MKRadio3/MKRadio4
    // Techem FHKV.
    case 0x80:
      return "Heat Cost Allocator"; // FHKV data ii/iii
...
std::string mediaTypeJSON(int a_field_device_type, int m_field) {
  ...
  case 0x0a:
    return "cooling load volume at outlet";
  case 0x0b:
    return "cooling load volume at inlet";
  case 0x0c:
    return "heat volume at inlet";
  case 0x0d:
...
#define LIST_OF_CI_FIELDS                                                      \
  ...
  X(0x8F, ELL_IV, "ELL: IV", 16, CI_TYPE::ELL,                                 \
    "CC, ACC, M2, A2, SN, Payload CRC")                                        \
...
#define X(val, name, cname, len, citype, explain)                              \
  if (ci_field == val && type == citype)                                       \
    return true;
  LIST_OF_CI_FIELDS
#undef X
  return false;
...
void Telegram::addMoreExplanation(int pos, const char *fmt, ...) {
  ...
  for (auto &p : explanations) {
    if (p.pos == pos) {
      ...
      p.info = p.info + buf;
      // Since we are adding more information, we assume that we have a full
      // understanding.
  ...
  if (!found) {
    debug("(wmbus) warning: cannot find offset %d to add more explanation "
          "\"%s\"\n",
          pos, buf);
  }
#endif
...
bool Telegram::parseMBusDLLandTPL(std::vector<uchar>::iterator &pos) {
  ...
  if (*pos != 0x68)
    return false;
  addExplanationAndIncrementPos(pos, 1, KindOfData::PROTOCOL,
                                Understanding::FULL, "68 start");

  dll_len = *pos;
  addExplanationAndIncrementPos(pos, 1, KindOfData::PROTOCOL,
                                Understanding::FULL, "%02x length (%d bytes)",
                                dll_len, dll_len);
  ...
  addExplanationAndIncrementPos(
      pos, 1, KindOfData::PROTOCOL, Understanding::FULL,
      "%02x length again (%d bytes)", dll_len, dll_len);
  ...
  // Second last byte should be crc. Should have been checked before! No need to
  // check again here?
  end--;
  setExplanation(end, 1, KindOfData::PROTOCOL, Understanding::FULL, "%02X crc",
                 *end);
  ...
  addExplanationAndIncrementPos(pos, 1, KindOfData::PROTOCOL,
                                Understanding::FULL, "%02x dll-a primary (%d)",
                                mbus_primary_address, mbus_primary_address);

  // Add dll_id to ids.
  std::string id = tostrprintf("p%d", mbus_primary_address);
  Address a;
  a.id = id;
  addresses.push_back(a);
  ...
  mbus_ci = *pos;
  addExplanationAndIncrementPos(pos, 1, KindOfData::PROTOCOL,
                                Understanding::FULL, "%02x tpl-ci (%s)",
                                mbus_ci, mbusCiField(mbus_ci));
...
bool Telegram::parseDLL(std::vector<uchar>::iterator &pos) {
  int remaining = std::distance(pos, frame.end());
  if (remaining == 0)
    return expectedMore(__LINE__);
  ...
  addExplanationAndIncrementPos(pos, 2, KindOfData::PROTOCOL,
                                Understanding::FULL, "%02x%02x dll-mfct (%s)",
  ...
  // Add dll_id to ids.
  addExplanationAndIncrementPos(
      pos, 4, KindOfData::PROTOCOL, Understanding::FULL,
      "%02x%02x%02x%02x dll-id (%s)", *(pos + 0), *(pos + 1), *(pos + 2),
      *(pos + 3), addresses.back().id.c_str());

  dll_version = *(pos + 0);
  dll_type = *(pos + 1);
  addExplanationAndIncrementPos(pos, 1, KindOfData::PROTOCOL,
                                Understanding::FULL, "%02x dll-version",
                                dll_version);
  addExplanationAndIncrementPos(
      pos, 1, KindOfData::PROTOCOL, Understanding::FULL, "%02x dll-type (%s)",
      dll_type, mediaType(dll_type, dll_mfct).c_str());

  return true;
}

std::string Telegram::toStringFromELLSN(int sn) {
...
bool Telegram::parseELL(std::vector<uchar>::iterator &pos) {
  int remaining = std::distance(pos, frame.end());
  if (remaining == 0)
    return false;
  ...
  ell_acc = *pos;
  addExplanationAndIncrementPos(pos, 1, KindOfData::PROTOCOL,
                                Understanding::FULL, "%02x ell-acc", ell_acc);
  ...
  if (has_target_mft_address) {
    ...
    addExplanationAndIncrementPos(pos, 2, KindOfData::PROTOCOL,
                                  Understanding::FULL, "%02x%02x ell-mfct (%s)",
    ...
    // Add ell_id to ids.
    addExplanationAndIncrementPos(pos, 4, KindOfData::PROTOCOL,
                                  Understanding::FULL,
                                  "%02x%02x%02x%02x ell-id", ell_id_b[0],
                                  ell_id_b[1], ell_id_b[2], ell_id_b[3]);

    ell_version = *pos;
    addExplanationAndIncrementPos(pos, 1, KindOfData::PROTOCOL,
                                  Understanding::FULL, "%02x ell-version",
                                  ell_version);

    ell_type = *pos;
    addExplanationAndIncrementPos(pos, 1, KindOfData::PROTOCOL,
                                  Understanding::FULL, "%02x ell-type",
                                  ell_type);
  }
  ...
  if (has_session_number_pl_crc) {
    ...
    ell_sn_b[3] = *(pos + 3);
    ell_sn =
        ell_sn_b[3] << 24 | ell_sn_b[2] << 16 | ell_sn_b[1] << 8 | ell_sn_b[0];
    ...
    std::string info = toString(ell_sec_mode);
    addExplanationAndIncrementPos(
        pos, 4, KindOfData::PROTOCOL, Understanding::FULL,
        "%02x%02x%02x%02x sn (%s)", ell_sn_b[0], ell_sn_b[1], ell_sn_b[2],
        ell_sn_b[3], info.c_str());

    if (ell_sec_mode == ELLSecurityMode::AES_CTR) {
      if (meter_keys) {
        decrypt_ELL_AES_CTR(this, frame, pos, meter_keys->confidentiality_key);
        // Actually this ctr decryption always succeeds, if wrong key, it will
        // decrypt to garbage.
      }
    ...
    int dist = std::distance(frame.begin(), pos + 2);
    int len = std::distance(pos + 2, frame.end());
    uint16_t check = crc16_EN13757(&(frame[dist]), len);

    addExplanationAndIncrementPos(
        pos, 2, KindOfData::PROTOCOL, Understanding::FULL,
        "%02x%02x payload crc (calculated %02x%02x %s)", ell_pl_crc_b[0],
        ell_pl_crc_b[1], check & 0xff, check >> 8,
        (ell_pl_crc == check ? "OK" : "ERROR"));
    ...
    if (ell_pl_crc == check || FUZZING) {
      ...
      std::string info = bin2hex(pos, frame.end(), num_encrypted_bytes);
      info += " failed decryption. Wrong key?";
      addExplanationAndIncrementPos(pos, num_encrypted_bytes,
                                    KindOfData::CONTENT,
                                    Understanding::ENCRYPTED, info.c_str());

      if (parser_warns_) {
        if (!beingAnalyzed()) {
          // Print this warning only once! Unless you are using verbose or
          // debug.
          verbose("(wmbus) WARNING! decrypted payload crc failed check, did "
                  "you use the correct decryption key? "
                  "%02x%02x payload crc (calculated %02x%02x) "
                  "Permanently ignoring telegrams from id: %02x%02x%02x%02x "
                  "mfct: (%s) %s (0x%02x) type: %s (0x%02x) ver: 0x%02x\n",
                  ell_pl_crc_b[0], ell_pl_crc_b[1], check & 0xff, check >> 8,
                  dll_id_b[3], dll_id_b[2], dll_id_b[1], dll_id_b[0],
                  manufacturerFlag(dll_mfct).c_str(),
                  manufacturer(dll_mfct).c_str(), dll_mfct,
                  mediaType(dll_type, dll_mfct).c_str(), dll_type, dll_version);
        }
      }
    }
  }
...
bool Telegram::parseNWL(std::vector<uchar>::iterator &pos) {
  int remaining = std::distance(pos, frame.end());
  if (remaining == 0)
    return false;
  ...
  uchar nwl = *pos;
  addExplanationAndIncrementPos(pos, 1, KindOfData::PROTOCOL,
                                Understanding::FULL, "%02x nwl?", nwl);

  return true;
}

bool Telegram::parseAFL(std::vector<uchar>::iterator &pos) {
  // 90 0F (len) 002C (fc) 25 (mc) 49EE 0A00 77C1 9D3D 1A08 ABCD ---
  // 729067296179161102F 90 0F (len) 002C (fc) 25 (mc) 0C39 0000 ED17 6BBB B159
  // 1ADB --- 7A1D003007103EA
  ...
  afl_len = *pos;
  addExplanationAndIncrementPos(pos, 1, KindOfData::PROTOCOL,
                                Understanding::FULL, "%02x afl-len (%d)",
                                afl_len, afl_len);
  ...
  std::string afl_fc_info = toStringFromAFLFC(afl_fc);
  addExplanationAndIncrementPos(pos, 2, KindOfData::PROTOCOL,
                                Understanding::FULL, "%02x%02x afl-fc (%s)",
  ...
  if (has_control) {
    afl_mcl = *pos;
    std::string afl_mcl_info = toStringFromAFLMC(afl_mcl);
    addExplanationAndIncrementPos(pos, 1, KindOfData::PROTOCOL,
                                  Understanding::FULL, "%02x afl-mcl (%s)",
  ...
  if (has_key_info) {
    ...
    std::string afl_ki_info = "";
    addExplanationAndIncrementPos(
        pos, 2, KindOfData::PROTOCOL, Understanding::FULL,
        "%02x%02x afl-ki (%s)", afl_ki_b[0], afl_ki_b[1], afl_ki_info.c_str());
  }
  ...
  if (has_counter) {
    ...
    afl_counter = afl_counter_b[3] << 24 | afl_counter_b[2] << 16 |
                  afl_counter_b[1] << 8 | afl_counter_b[0];

    addExplanationAndIncrementPos(
        pos, 4, KindOfData::PROTOCOL, Understanding::FULL,
        "%02x%02x%02x%02x afl-counter (%u)", afl_counter_b[0], afl_counter_b[1],
        afl_counter_b[2], afl_counter_b[3], afl_counter);
  }
  ...
  if (has_mac) {
    ...
    int len = toLen(aat);
    if (len != 2 && len != 4 && len != 8 && len != 12 && len != 16) {
      if (parser_warns_) {
    ...
    std::string s = bin2hex(afl_mac_b);
    addExplanationAndIncrementPos(pos, len, KindOfData::PROTOCOL,
                                  Understanding::FULL, "%s afl-mac %d bytes",
                                  s.c_str(), len);
    must_check_mac = true;
  }
...
std::string Telegram::toStringFromAFLFC(int fc) {
  std::string info = "";
  int fid = fc & 0x00ff; // Fragmend id
  info += std::to_string(fid);
  info += " ";
  if (fc & 0x0200)
    info += "KeyInfoInFragment ";
  if (fc & 0x0400)
    info += "MACInFragment ";
  if (fc & 0x0800)
    info += "MessCounterInFragment ";
  if (fc & 0x1000)
    info += "MessLenInFragment ";
  if (fc & 0x2000)
    info += "MessControlInFragment ";
  if (fc & 0x4000)
    info += "MoreFragments ";
  else
    info += "LastFragment ";
...
std::string Telegram::toStringFromAFLMC(int mc) {
  std::string info = "";
  int at = mc & 0x0f;
  AFLAuthenticationType aat = fromIntToAFLAuthenticationType(at);
  info += toString(aat);
...
std::string Telegram::toStringFromTPLConfig(int cfg) {
  std::string info = "";
  if (cfg & 0x8000)
    info += "bidirectional ";
  if (cfg & 0x4000)
...
bool Telegram::parseTPLConfig(std::vector<uchar>::iterator &pos) {
  ...
  std::string info = toStringFromTPLConfig(tpl_cfg);
  info += " ";
  ...
  if (has_cfg_ext) {
    ...
    if (tpl_kdf_selection == 1) {
      ...
      input.insert(
          input.end(),
          0x00); // DC 00 = generate ephemereal encryption key from meter.
      ...
      AES_CMAC(safeButUnsafeVectorPtr(meter_keys->confidentiality_key),
               safeButUnsafeVectorPtr(input), 16, safeButUnsafeVectorPtr(mac));
      std::string s = bin2hex(mac);
      ...
      tpl_generated_mac_key.insert(tpl_generated_mac_key.end(), mac.begin(),
                                   mac.end());
    }
  }
...
bool Telegram::parseShortTPL(std::vector<uchar>::iterator &pos) {
  ...
  tpl_acc = *pos;
  addExplanationAndIncrementPos(pos, 1, KindOfData::PROTOCOL,
                                Understanding::FULL, "%02x tpl-acc-field",
                                tpl_acc);

  CHECK(1);
  tpl_sts = *pos;
  tpl_sts_offset = std::distance(frame.begin(), pos);
  addExplanationAndIncrementPos(
      pos, 1, KindOfData::PROTOCOL, Understanding::FULL,
      "%02x tpl-sts-field (%s)", tpl_sts,
      decodeTPLStatusByteOnlyStandardBits(tpl_sts).c_str());
...
bool Telegram::parseLongTPL(std::vector<uchar>::iterator &pos) {
  ...
  addExplanationAndIncrementPos(
      pos, 4, KindOfData::PROTOCOL, Understanding::FULL,
      "%02x%02x%02x%02x tpl-id (%02x%02x%02x%02x)", tpl_id_b[0], tpl_id_b[1],
      tpl_id_b[2], tpl_id_b[3], tpl_id_b[3], tpl_id_b[2], tpl_id_b[1],
      tpl_id_b[0]);
  ...
  std::string man = manufacturerFlag(tpl_mfct);
  addExplanationAndIncrementPos(pos, 2, KindOfData::PROTOCOL,
                                Understanding::FULL, "%02x%02x tpl-mfct (%s)",
                                tpl_mfct_b[0], tpl_mfct_b[1], man.c_str());

  CHECK(1);
  tpl_version = *(pos + 0);
  tpl_a[4] = *(pos + 0);
  addExplanationAndIncrementPos(pos, 1, KindOfData::PROTOCOL,
                                Understanding::FULL, "%02x tpl-version",
                                tpl_version);
  ...
  addExplanationAndIncrementPos(pos, 1, KindOfData::PROTOCOL,
                                Understanding::FULL, "%02x tpl-type (%s)",
                                tpl_type, info.c_str());
...
bool Telegram::checkMAC(std::vector<uchar> &frame,
                        ...
                        std::vector<uchar> &inmac, std::vector<uchar> &mackey) {
  std::vector<uchar> input;
  std::vector<uchar> mac;
  mac.resize(16);
  ...
  //                 AFL.MCL || AFL.MCR || {AFL.ML || } NextCI || ... || Last
  //                 Byte of message)

  input.insert(input.end(), afl_mcl);
  input.insert(input.end(), afl_counter_b, afl_counter_b + 4);
  input.insert(input.end(), from, to);
  std::string s = bin2hex(input);
  debug("(wmbus) input to mac %s\n", s.c_str());
  AES_CMAC(safeButUnsafeVectorPtr(mackey), safeButUnsafeVectorPtr(input),
           input.size(), safeButUnsafeVectorPtr(mac));
  std::string calculated = bin2hex(mac);
  debug("(wmbus) calculated mac %s\n", calculated.c_str());
  std::string received = bin2hex(inmac);
  debug("(wmbus) received   mac %s\n", received.c_str());
  std::string truncated = calculated.substr(0, received.length());
...
bool Telegram::alreadyDecryptedCBC(std::vector<uchar>::iterator &pos) {
  CHECK(2);
...
bool Telegram::potentiallyDecrypt(std::vector<uchar>::iterator &pos) {
  if (tpl_sec_mode == TPLSecurityMode::AES_CBC_IV) {
    if (alreadyDecryptedCBC(pos)) {
      if (meter_keys && meter_keys->hasConfidentialityKey()) {
        // Oups! This telegram is already decrypted (but the header still says
        // it should be encrypted) this is probably a replay telegram from
        // --logtelegrams. But since we have specified a key! Do not accept this
        // telegram!
        warning("(wmbus) WARNING!! telegram should have been fully encrypted, "
                "but was not! "
                "id: %02x%02x%02x%02x mfct: (%s) %s (0x%02x) type: %s (0x%02x) "
                "ver: 0x%02x\n",
                dll_id_b[3], dll_id_b[2], dll_id_b[1], dll_id_b[0],
                manufacturerFlag(dll_mfct).c_str(),
                manufacturer(dll_mfct).c_str(), dll_mfct,
    ...
    bool ok = decrypt_TPL_AES_CBC_IV(
        this, frame, pos, meter_keys->confidentiality_key, &num_encrypted_bytes,
    ...
    addExplanationAndIncrementPos(
        ...
        (*(pos + 0) == 0x2f && *(pos + 1) == 0x2f) ? "OK"
                                                   : "ERROR should be 2f2f");

    if ((a != 0x2f || b != 0x2f) && !FUZZING) {
      // Wrong key supplied.
      int num_bytes = std::distance(pos, frame.end());
      std::string info = bin2hex(pos, frame.end(), num_bytes);
      info += " failed decryption. Wrong key?";
      addExplanationAndIncrementPos(pos, num_bytes, KindOfData::CONTENT,
                                    Understanding::ENCRYPTED, info.c_str());
      ...
      if (parser_warns_) {
        if (!beingAnalyzed()) {
          // Print this warning only once! Unless you are using verbose or
          // debug.
          warning("(wmbus) WARNING!! decrypted content failed check, did you "
                  "use the correct decryption key? "
                  "Permanently ignoring telegrams from id: %02x%02x%02x%02x "
                  "mfct: (%s) %s (0x%02x) type: %s (0x%02x) ver: 0x%02x\n",
                  dll_id_b[3], dll_id_b[2], dll_id_b[1], dll_id_b[0],
                  manufacturerFlag(dll_mfct).c_str(),
    ...
    bool mac_ok = checkMAC(frame, tpl_start, frame.end(), afl_mac_b,
                           tpl_generated_mac_key);
    ...
    if (!mac_ok) {
      if (parser_warns_) {
        if (!beingAnalyzed()) {
          // Print this warning only once! Unless you are using verbose or
          // debug.
          warning("(wmbus) WARNING! telegram mac check failed, did you use the "
        ...
        std::string info = bin2hex(pos, frame.end(), frame.end() - pos);
        info += " encrypted mac failed";
        addExplanationAndIncrementPos(pos, frame.end() - pos,
                                      KindOfData::CONTENT,
                                      Understanding::ENCRYPTED, info.c_str());
    ...
    if (mustDecryptDiehlRealData(frame)) {
      ...
      if (!meter_keys)
        return false;
      bool ok = decryptDielhRealData(this, frame, pos,
                                     meter_keys->confidentiality_key);
      // If this telegram is simulated, the content might already be decrypted
      // and the decruption will fail. But we can assume all is well anyway!
      if (!ok && isSimulated())
        return true;
      if (!ok)
        return false;
  ...
  } else if (meter_keys && meter_keys->hasConfidentialityKey()) {
    // Oups! This telegram is NOT encrypted, but we have specified a key!
    // Do not accept this telegram!
    warning(
        "(wmbus) WARNING!!! telegram should have been encrypted, but was not! "
        "id: %02x%02x%02x%02x mfct: (%s) %s (0x%02x) type: %s (0x%02x) ver: "
        "0x%02x\n",
        dll_id_b[3], dll_id_b[2], dll_id_b[1], dll_id_b[0],
        manufacturerFlag(dll_mfct).c_str(), manufacturer(dll_mfct).c_str(),
        dll_mfct, mediaType(dll_type, dll_mfct).c_str(), dll_type, dll_version);
    return false;
  }
...
bool Telegram::parse_TPL_72(std::vector<uchar>::iterator &pos) {
  ...
  header_size = std::distance(frame.begin(), pos);
  int remaining = std::distance(pos, frame.end()) - suffix_size;
...
bool Telegram::parse_TPL_78(std::vector<uchar>::iterator &pos) {
  header_size = std::distance(frame.begin(), pos);
  int remaining = std::distance(pos, frame.end()) - suffix_size;
  parseDV(this, frame, pos, remaining, &dv_entries);
...
bool Telegram::parse_TPL_79(std::vector<uchar>::iterator &pos) {
  // Compact frame
  CHECK(2);
  uchar ecrc0 = *(pos + 0);
  uchar ecrc1 = *(pos + 1);
  size_t offset = std::distance(frame.begin(), pos);
  addExplanationAndIncrementPos(pos, 2, KindOfData::PROTOCOL,
                                Understanding::FULL,
  ...
  if (!ok) {
    ...
    if (!ok) {
      ...
      int num_compressed_bytes = std::distance(pos, frame.end());
      std::string info = bin2hex(pos, frame.end(), num_compressed_bytes);
      info += " compressed and signature unknown";
      addExplanationAndIncrementPos(pos, std::distance(pos, frame.end()),
                                    KindOfData::CONTENT,
                                    Understanding::COMPRESSED, info.c_str());

      verbose("(wmbus) ignoring compressed telegram since format signature "
              "hash 0x%02x is yet unknown.\n"
              "     this is not a problem, since you only need wait for at "
              "most 8 telegrams\n"
              "     (8*16 seconds) until an full length telegram arrives and "
              "then we know\n"
              "     the format giving this hash and start decoding the "
              "telegrams properly.\n",
              format_signature);
      return false;
    }
  }
  std::vector<uchar>::iterator format = format_bytes.begin();

  // 2,3 = crc for payload = hash over both DRH and data bytes. Or is it only
  // over the data bytes?
  CHECK(2);
  int ecrc2 = *(pos + 0);
  int ecrc3 = *(pos + 1);
  addExplanationAndIncrementPos(pos, 2, KindOfData::PROTOCOL,
                                Understanding::FULL, "%02x%02x data crc", ecrc2,
                                ecrc3);

  header_size = std::distance(frame.begin(), pos);
  int remaining = std::distance(pos, frame.end()) - suffix_size;

  parseDV(this, frame, pos, remaining, &dv_entries, &format,
          format_bytes.size());

  return true;
}

bool Telegram::parse_TPL_7A(std::vector<uchar>::iterator &pos) {
  bool ok = parseShortTPL(pos);
  if (!ok)
    return false;
...
bool Telegram::parseTPL(std::vector<uchar>::iterator &pos) {
  int remaining = std::distance(pos, frame.end());
  if (remaining == 0)
    return false;
  ...
  if (!isCiFieldOfType(ci_field, CI_TYPE::TPL) && !mfct_specific) {
    addExplanationAndIncrementPos(pos, 1, KindOfData::PROTOCOL,
                                  Understanding::NONE, "%02x unknown ci-field",
                                  ci_field);
    if (parser_warns_) {
  ...
  default: {
    ...
    std::string info = bin2hex(pos, frame.end(), num_mfct_bytes);
    info += " mfct specific";
    addExplanationAndIncrementPos(pos, num_mfct_bytes, KindOfData::CONTENT,
                                  Understanding::NONE, info.c_str());
...
void Telegram::preProcess() {
  ...
  if (diehl_method != DiehlAddressTransformMethod::NONE) {
    ...
    original = std::vector<uchar>(frame.begin(), frame.begin() + 10);
    transformDiehlAddress(frame, diehl_method);
  }
}

bool Telegram::parse(std::vector<uchar> &input_frame, MeterKeys *mk,
                     bool warn) {
  switch (about.type) {
  case FrameType::WMBUS:
    return parseWMBUS(input_frame, mk, warn);
  case FrameType::MBUS:
    return parseMBUS(input_frame, mk, warn);
  case FrameType::HAN:
    return parseHAN(input_frame, mk, warn);
  }
  assert(0);
  return false;
}

bool Telegram::parseHeader(std::vector<uchar> &input_frame) {
  switch (about.type) {
  case FrameType::WMBUS:
    return parseWMBUSHeader(input_frame);
  case FrameType::MBUS:
    return parseMBUSHeader(input_frame);
  case FrameType::HAN:
    return parseHANHeader(input_frame);
  }
  assert(0);
  return false;
}

bool Telegram::parseWMBUSHeader(std::vector<uchar> &input_frame) {
  ...
  parsed.clear();
  // Fixes quirks from non-compliant meters to make telegram compatible with the
  // standard
  preProcess();
...
bool Telegram::parseWMBUS(std::vector<uchar> &input_frame, MeterKeys *mk,
                          bool warn) {
  assert(about.type == FrameType::WMBUS);
...
bool Telegram::parseMBUSHeader(std::vector<uchar> &input_frame) {
...
bool Telegram::parseMBUS(std::vector<uchar> &input_frame, MeterKeys *mk,
                         bool warn) {
  assert(about.type == FrameType::MBUS);
...
bool Telegram::parseHANHeader(std::vector<uchar> &input_frame) {
  assert(about.type == FrameType::HAN);

  return false;
}

bool Telegram::parseHAN(std::vector<uchar> &input_frame, MeterKeys *mk,
                        bool warn) {
  assert(about.type == FrameType::HAN);
...
const char *mbusCiField(uchar c_field) {
  ...
  case 0x72:
    return "long header";
  case 0x79:
    return "no header compact frame";
  case 0x7b:
    return "short header compact frame";
  case 0x73:
    return "long header compact frame";
  case 0x69:
    return "no header format frame";
  case 0x6a:
    return "short header format frame";
  case 0x6b:
    return "long header format frame";
  }
  return "?";
}
...
bool isValidWMBusCField(int c_field) {
  ...
  // However in the future we might see relayed telegrams which will perhaps
  // have some other c field.
...
int difLenBytes(int dif) {
  ...
  case 0x0:
    return 0; // No data
  case 0x1:
    return 1; // 8 Bit Integer/Binary
  case 0x2:
    return 2; // 16 Bit Integer/Binary
  case 0x3:
    return 3; // 24 Bit Integer/Binary
  case 0x4:
    return 4; // 32 Bit Integer/Binary
  case 0x5:
    return 4; // 32 Bit Real
  case 0x6:
    return 6; // 48 Bit Integer/Binary
  case 0x7:
    return 8; // 64 Bit Integer/Binary
  case 0x8:
  ...
  case 0xF:   // Special Functions
    if (dif == 0x2f)
      return 1; // The skip code 0x2f, used for padding.
    return -2;
  }
  // Bad!
  return -2;
...
std::string vifType(int vif) {
  ...
  case 0x20:
    return "On time seconds";
  case 0x21:
    return "On time minutes";
  case 0x22:
    return "On time hours";
  case 0x23:
    return "On time days";

  case 0x24:
    return "Operating time seconds";
  case 0x25:
    return "Operating time minutes";
  case 0x26:
    return "Operating time hours";
  case 0x27:
    return "Operating time days";
  ...
  case 0x38:
    return "Volume flow cm³/h";
  case 0x39:
    return "Volume flow 10⁻⁵ m³/h";
  case 0x3A:
    return "Volume flow 10⁻⁴ m³/h";
  case 0x3B:
  ...
  case 0x3D:
    return "Volume flow 10⁻¹ m³/h";
  case 0x3E:
    return "Volume flow m³/h";
  case 0x3F:
    return "Volume flow 10¹ m³/h";

  case 0x40:
    return "Volume flow ext. 10⁻⁷ m³/min";
  case 0x41:
    return "Volume flow ext. cm³/min";
  case 0x42:
    return "Volume flow ext. 10⁻⁵ m³/min";
  case 0x43:
    return "Volume flow ext. 10⁻⁴ m³/min";
  case 0x44:
    return "Volume flow ext. l/min";
  case 0x45:
    return "Volume flow ext. 10⁻² m³/min";
  case 0x46:
    return "Volume flow ext. 10⁻¹ m³/min";
  case 0x47:
    return "Volume flow ext. m³/min";

  case 0x48:
    return "Volume flow ext. mm³/s";
  case 0x49:
    return "Volume flow ext. 10⁻⁸ m³/s";
  case 0x4A:
    return "Volume flow ext. 10⁻⁷ m³/s";
  case 0x4B:
    return "Volume flow ext. cm³/s";
  case 0x4C:
    return "Volume flow ext. 10⁻⁵ m³/s";
  case 0x4D:
    return "Volume flow ext. 10⁻⁴ m³/s";
  case 0x4E:
    return "Volume flow ext. l/s";
  case 0x4F:
    return "Volume flow ext. 10⁻² m³/s";
  ...
  case 0x5C:
    return "Return temperature 10⁻³ °C";
  case 0x5D:
    return "Return temperature 10⁻² °C";
  case 0x5E:
    return "Return temperature 10⁻¹ °C";
  ...
  case 0x60:
    return "Temperature difference 10⁻³ K/°C";
  case 0x61:
    return "Temperature difference 10⁻² K/°C";
  case 0x62:
    return "Temperature difference 10⁻¹ K/°C";
  case 0x63:
    return "Temperature difference K/°C";

  case 0x64:
    return "External temperature 10⁻³ °C";
  case 0x65:
    return "External temperature 10⁻² °C";
  case 0x66:
    return "External temperature 10⁻¹ °C";
  case 0x67:
    return "External temperature °C";

  case 0x68:
    return "Pressure mbar";
  case 0x69:
    return "Pressure 10⁻² bar";
  case 0x6A:
    return "Pressure 10⁻¹ bar";
  case 0x6B:
    return "Pressure bar";
  ...
  case 0x6C:
    return "Date type G";
  case 0x6D:
    return "Date and time type";

  case 0x6E:
    return "Units for H.C.A.";
  case 0x6F:
    return "Third extension 6F of VIF-codes";

  case 0x70:
    return "Averaging duration seconds";
  case 0x71:
    return "Averaging duration minutes";
  case 0x72:
    return "Averaging duration hours";
  case 0x73:
    return "Averaging duration days";

  case 0x74:
    return "Actuality duration seconds";
  case 0x75:
    return "Actuality duration minutes";
  case 0x76:
    return "Actuality duration hours";
  case 0x77:
    return "Actuality duration days";
  ...
  case 0x7B:
    return "First extension FB of VIF-codes";
  case 0x7C:
    return "VIF in following string (length in first byte)";
  case 0x7D:
    return "Second extension FD of VIF-codes";
...
double vifScale(int vif) {
  ...
  case 0x00:
    return 1000000.0; // Energy mWh
  case 0x01:
    return 100000.0; // Energy 10⁻² Wh
  case 0x02:
    return 10000.0; // Energy 10⁻¹ Wh
  case 0x03:
    return 1000.0; // Energy Wh
  case 0x04:
    return 100.0; // Energy 10¹ Wh
  case 0x05:
    return 10.0; // Energy 10² Wh
  case 0x06:
    return 1.0; // Energy kWh
  case 0x07:
    return 0.1; // Energy 10⁴ Wh

    // or wmbusmeters always returns energy as MJ
  case 0x08:
    return 1000000.0; // Energy J
  case 0x09:
    return 100000.0; // Energy 10¹ J
  case 0x0A:
    return 10000.0; // Energy 10² J
  case 0x0B:
    return 1000.0; // Energy kJ
  case 0x0C:
    return 100.0; // Energy 10⁴ J
  case 0x0D:
    return 10.0; // Energy 10⁵ J
  case 0x0E:
    return 1.0; // Energy MJ
  case 0x0F:
    return 0.1; // Energy 10⁷ J

    // wmbusmeters always returns volume as m3
  case 0x10:
    return 1000000.0; // Volume cm³
  case 0x11:
    return 100000.0; // Volume 10⁻⁵ m³
  case 0x12:
    return 10000.0; // Volume 10⁻⁴ m³
  case 0x13:
    return 1000.0; // Volume l
  case 0x14:
    return 100.0; // Volume 10⁻² m³
  case 0x15:
    return 10.0; // Volume 10⁻¹ m³
  case 0x16:
    return 1.0; // Volume m³
  case 0x17:
    return 0.1; // Volume 10¹ m³

    // wmbusmeters always returns weight in kg
  case 0x18:
    return 1000.0; // Mass g
  case 0x19:
    return 100.0; // Mass 10⁻² kg
  case 0x1A:
    return 10.0; // Mass 10⁻¹ kg
  case 0x1B:
    return 1.0; // Mass kg
  case 0x1C:
    return 0.1; // Mass 10¹ kg
  case 0x1D:
    return 0.01; // Mass 10² kg
  case 0x1E:
    return 0.001; // Mass t
  case 0x1F:
    return 0.0001; // Mass 10⁴ kg

    // wmbusmeters always returns time in hours
  case 0x20:
    return 3600.0; // On time seconds
  case 0x21:
    return 60.0; // On time minutes
  case 0x22:
    return 1.0; // On time hours
  case 0x23:
    return (1.0 / 24.0); // On time days

  case 0x24:
    return 3600.0; // Operating time seconds
  case 0x25:
    return 60.0; // Operating time minutes
  case 0x26:
    return 1.0; // Operating time hours
  case 0x27:
    return (1.0 / 24.0); // Operating time days

    // wmbusmeters always returns power in kw
  case 0x28:
    return 1000000.0; // Power mW
  case 0x29:
    return 100000.0; // Power 10⁻² W
  case 0x2A:
    return 10000.0; // Power 10⁻¹ W
  case 0x2B:
    return 1000.0; // Power W
  case 0x2C:
    return 100.0; // Power 10¹ W
  case 0x2D:
    return 10.0; // Power 10² W
  case 0x2E:
    return 1.0; // Power kW
  case 0x2F:
    return 0.1; // Power 10⁴ W

    // or wmbusmeters always returns power in MJh
  case 0x30:
    return 1000000.0; // Power J/h
  case 0x31:
    return 100000.0; // Power 10¹ J/h
  case 0x32:
    return 10000.0; // Power 10² J/h
  case 0x33:
    return 1000.0; // Power kJ/h
  case 0x34:
    return 100.0; // Power 10⁴ J/h
  case 0x35:
    return 10.0; // Power 10⁵ J/h
  case 0x36:
    return 1.0; // Power MJ/h
  case 0x37:
    return 0.1; // Power 10⁷ J/h

    // wmbusmeters always returns volume flow in m3h
  case 0x38:
    return 1000000.0; // Volume flow cm³/h
  case 0x39:
    return 100000.0; // Volume flow 10⁻⁵ m³/h
  case 0x3A:
    return 10000.0; // Volume flow 10⁻⁴ m³/h
  case 0x3B:
    return 1000.0; // Volume flow l/h
  case 0x3C:
    return 100.0; // Volume flow 10⁻² m³/h
  case 0x3D:
    return 10.0; // Volume flow 10⁻¹ m³/h
  case 0x3E:
    return 1.0; // Volume flow m³/h
  case 0x3F:
    return 0.1; // Volume flow 10¹ m³/h

    // wmbusmeters always returns volume flow in m3h
  case 0x40:
    return 600000000.0; // Volume flow ext. 10⁻⁷ m³/min
  case 0x41:
    return 60000000.0; // Volume flow ext. cm³/min
  case 0x42:
    return 6000000.0; // Volume flow ext. 10⁻⁵ m³/min
  case 0x43:
    return 600000.0; // Volume flow ext. 10⁻⁴ m³/min
  case 0x44:
    return 60000.0; // Volume flow ext. l/min
  case 0x45:
    return 6000.0; // Volume flow ext. 10⁻² m³/min
  case 0x46:
    return 600.0; // Volume flow ext. 10⁻¹ m³/min
  case 0x47:
    return 60.0; // Volume flow ext. m³/min
  ...
  case 0x48:
    return 1000000000.0 * 3600; // Volume flow ext. mm³/s
  case 0x49:
    return 100000000.0 * 3600; // Volume flow ext. 10⁻⁸ m³/s
  case 0x4A:
    return 10000000.0 * 3600; // Volume flow ext. 10⁻⁷ m³/s
  case 0x4B:
    return 1000000.0 * 3600; // Volume flow ext. cm³/s
  case 0x4C:
    return 100000.0 * 3600; // Volume flow ext. 10⁻⁵ m³/s
  case 0x4D:
    return 10000.0 * 3600; // Volume flow ext. 10⁻⁴ m³/s
  case 0x4E:
    return 1000.0 * 3600; // Volume flow ext. l/s
  case 0x4F:
    return 100.0 * 3600; // Volume flow ext. 10⁻² m³/s

    // wmbusmeters always returns mass flow as kgh
  case 0x50:
    return 1000.0; // Mass g/h
  case 0x51:
    return 100.0; // Mass 10⁻² kg/h
  case 0x52:
    return 10.0; // Mass 10⁻¹ kg/h
  case 0x53:
    return 1.0; // Mass kg/h
  case 0x54:
    return 0.1; // Mass 10¹ kg/h
  case 0x55:
    return 0.01; // Mass 10² kg/h
  case 0x56:
    return 0.001; // Mass t/h
  case 0x57:
    return 0.0001; // Mass 10⁴ kg/h

    // wmbusmeters always returns temperature in °C
  case 0x58:
    return 1000.0; // Flow temperature 10⁻³ °C
  case 0x59:
    return 100.0; // Flow temperature 10⁻² °C
  case 0x5A:
    return 10.0; // Flow temperature 10⁻¹ °C
  case 0x5B:
    return 1.0; // Flow temperature °C

    // wmbusmeters always returns temperature in c
  case 0x5C:
    return 1000.0; // Return temperature 10⁻³ °C
  case 0x5D:
    return 100.0; // Return temperature 10⁻² °C
  case 0x5E:
    return 10.0; // Return temperature 10⁻¹ °C
  case 0x5F:
    return 1.0; // Return temperature °C

    // or if Kelvin is used as a temperature, in K
    // what kind of meter cares about -273.15 °C
    // a flow pump for liquid helium perhaps?
  case 0x60:
    return 1000.0; // Temperature difference mK
  case 0x61:
    return 100.0; // Temperature difference 10⁻² K
  case 0x62:
    return 10.0; // Temperature difference 10⁻¹ K
  case 0x63:
    return 1.0; // Temperature difference K

    // wmbusmeters always returns temperature in c
  case 0x64:
    return 1000.0; // External temperature 10⁻³ °C
  case 0x65:
    return 100.0; // External temperature 10⁻² °C
  case 0x66:
    return 10.0; // External temperature 10⁻¹ °C
  case 0x67:
    return 1.0; // External temperature °C

    // wmbusmeters always returns pressure in bar
  case 0x68:
    return 1000.0; // Pressure mbar
  case 0x69:
    return 100.0; // Pressure 10⁻² bar
  case 0x6A:
    return 10.0; // Pressure 10⁻¹ bar
  case 0x6B:
    return 1.0; // Pressure bar

  case 0x6C:
    return 1.0; // Date type G
  case 0x6D:
    return 1.0; // Date&Time type F
  case 0x6E:
    return 1.0; // Units for H.C.A. are never scaled
  case 0x6F:
    warning("(wmbus) warning: do not scale a reserved type!\n");
    return -1.0; // Reserved

    // wmbusmeters always returns time in hours
  case 0x70:
    return 3600.0; // Averaging duration seconds
  case 0x71:
    return 60.0; // Averaging duration minutes
  case 0x72:
    return 1.0; // Averaging duration hours
  case 0x73:
    return (1.0 / 24.0); // Averaging duration days

    // wmbusmeters always returns time in hours
  case 0x74:
    return 3600.0; // Actuality duration seconds
  case 0x75:
    return 60.0; // Actuality duration minutes
  case 0x76:
    return 1.0; // Actuality duration hours
  case 0x77:
    return (1.0 / 24.0); // Actuality duration days
  ...
  case 0x7b1a:
    return 10.0; // Relative humidity 0.1 %
  case 0x7b1b:
    return 1.0; // Relative humidity 1 %

    // wmbusmeters always returns time in hours
    // 0x7d30 is not supposed to be used according to spec.
  case 0x7d31:
    return 60.0; // Duration tariff minutes
  case 0x7d32:
    return 1.0; // Duration tariff hours
  case 0x7d33:
    return (1.0 / 24.0); // Duration tariff days
  ...
  case 0x7d74: {
    ...
    // wmbusmeters always returns time in hours
  case 0x7d2c:
    return 3600.0; // Duration since readout seconds
  case 0x7d2d:
    return 60.0; // Duration since readout minutes
  case 0x7d2e:
    return 1.0; // Duration since readout hours
  case 0x7d2f:
    return (1.0 / 24.0); // Duration since readout days
...
std::string vif_7B_FirstExtensionType(uchar dif, uchar vif, uchar vife) {
  assert(vif == 0xfb);
  ...
  if (((vife & 0x7e) == 0x02) || ((vife & 0x7c) == 0x04)) {
    return "Reserved";
  }
  ...
  if ((vife & 0x7e) == 0x0a || (vife & 0x7c) == 0x0c) {
    return "Reserved";
  }
  ...
  if ((vife & 0x7e) == 0x12 || (vife & 0x7c) == 0x14) {
    return "Reserved";
  }
  ...
  if ((vife & 0x7f) == 0x29 || (vife & 0x7c) == 0x2c) {
    return "Reserved";
  }
...
std::string vif_7D_SecondExtensionType(uchar dif, uchar vif, uchar vife) {
  assert(vif == 0xfd);

  if ((vife & 0x7c) == 0x00) {
    int nn = vife & 0x03;
    std::string s;
    strprintf(&s, "Credit of 10^%d of the nominal local legal currency units",
              nn - 3);
    return s;
  }
  ...
  if ((vife & 0x7c) == 0x04) {
    int nn = vife & 0x03;
    std::string s;
    strprintf(&s, "Debit of 10^%d of the nominal local legal currency units",
              nn - 3);
    return s;
  }
...
std::string vifeType(int dif, int vif, int vife) {
  ...
  if ((vife & 0x70) == 0x50) {
    std::string msg = "duration of limit exceed ";
    if (vife & 0x04)
      msg += "last ";
    else
      msg += "first ";
  ...
  if ((vife & 0x78) == 0x60) {
    std::string msg = "duration of a,b ";
    if (vife & 0x04)
      msg += "last ";
    else
      msg += "first ";
  ...
  if ((vife & 0x78) == 0x78) {
    int nn = vife & 0x03;
    return "additive correction constant: unit of VIF * 10^" +
           std::to_string(nn - 3);
  }
  if (vife == 0x7c) {
...
double toDoubleFromBytes(std::vector<uchar> &bytes, int len) {
  double d = 0;
  for (int i = 0; i < len; ++i) {
    double x = bytes[i];
    d += x * (256 ^ i);
...
double toDoubleFromBCD(std::vector<uchar> &bytes, int len) {
  double d = 0;
  for (int i = 0; i < len; ++i) {
...
double dataAsDouble(int dif, int vif, int vife, std::string data) {
  ...
  case 0x0:
    return 0.0;
  case 0x1:
    return toDoubleFromBytes(bytes, 1);
  case 0x2:
    return toDoubleFromBytes(bytes, 2);
  case 0x3:
    return toDoubleFromBytes(bytes, 3);
  case 0x4:
    return toDoubleFromBytes(bytes, 4);
  case 0x5:
    return -1; //  How is REAL stored?
  case 0x6:
    return toDoubleFromBytes(bytes, 6);
    ...
    // really needs true 64 bit precision in their measurements from a physical
    // meter though.
  case 0x7:
    return toDoubleFromBytes(bytes, 8);
  case 0x8:
    return -1; // Selection for Readout?
  case 0x9:
    return toDoubleFromBCD(bytes, 1);
  case 0xA:
    return toDoubleFromBCD(bytes, 2);
  case 0xB:
    return toDoubleFromBCD(bytes, 3);
  case 0xC:
    return toDoubleFromBCD(bytes, 4);
  case 0xD:
    return -1; // variable length
  case 0xE:
    return toDoubleFromBCD(bytes, 6);
  case 0xF:
...
uint64_t dataAsUint64(int dif, int vif, int vife, std::string data) {
  std::vector<uchar> bytes;
  hex2bin(data, &bytes);
...
bool Telegram::findFormatBytesFromKnownMeterSignatures(
    std::vector<uchar> *format_bytes) {
  bool ok = true;
...
void Telegram::extractMfctData(std::vector<uchar> *pl) {
  pl->clear();
  if (mfct_0f_index == -1)
    return;

  std::vector<uchar>::iterator from =
      frame.begin() + header_size + mfct_0f_index;
  std::vector<uchar>::iterator to = frame.end() - suffix_size;
  pl->insert(pl->end(), from, to);
}

void Telegram::extractPayload(std::vector<uchar> *pl) {
  pl->clear();
  std::vector<uchar>::iterator from = frame.begin() + header_size;
  std::vector<uchar>::iterator to = frame.end() - suffix_size;
  pl->insert(pl->end(), from, to);
}

void Telegram::extractFrame(std::vector<uchar> *fr) { *fr = frame; }

int toInt(AFLAuthenticationType aat) {
  switch (aat) {

#define X(name, nr, len)                                                       \
  case AFLAuthenticationType::name:                                            \
    return nr;
...
#define X(name, nr, len)                                                       \
  case nr:                                                                     \
    return AFLAuthenticationType::name;
    LIST_OF_AFL_AUTH_TYPES
#undef X
  }
...
bool trimCRCsFrameFormatAInternal(std::vector<uchar> &payload,
                                  bool fail_is_ok) {
  if (payload.size() < 12) {
    if (!fail_is_ok) {
  ...
  if (calc_crc != check_crc && !FUZZING) {
    if (!fail_is_ok) {
      debug("(wmbus) ff a dll crc first (calculated %04x) did not match "
            "(expected %04x) for bytes 0-%zu!\n",
  ...
  for (pos = 12; pos + 18 <= len; pos += 18) {
    ...
    if (calc_crc != check_crc && !FUZZING) {
      if (!fail_is_ok) {
        debug("(wmbus) ff a dll crc mid (calculated %04x) did not match "
              "(expected %04x) for bytes %zu-%zu!\n",
  ...
  if (pos < len - 2) {
    ...
    if (calc_crc != check_crc && !FUZZING) {
      if (!fail_is_ok) {
        debug("(wmbus) ff a dll crc final (calculated %04x) did not match "
              "(expected %04x) for bytes %zu-%zu!\n",
  ...
  debug("(wmbus) trimmed %zu dll crc bytes from frame a and ignored %zu suffix "
        "bytes.\n",
...
bool trimCRCsFrameFormatBInternal(std::vector<uchar> &payload,
                                  bool fail_is_ok) {
  if (payload.size() < 12) {
    if (!fail_is_ok) {
      debug("(wmbus) not enough bytes! expected at least 12 but got (%zu)!\n",
  ...
  if (calc_crc != check_crc && !FUZZING) {
    if (!fail_is_ok) {
      debug("(wmbus) ff b dll crc (calculated %04x) did not match (expected "
            "%04x) for bytes 0-%zu!\n",
  ...
  if (crc2_pos > 0) {
    ...
    out.insert(out.end(), payload.begin() + crc1_pos + 2,
               payload.begin() + crc2_pos);
    if (!fail_is_ok) {
  ...
  debug("(wmbus) trimmed %zu dll crc bytes from frame b and ignored %zu suffix "
        "bytes.\n",
        (len - new_len), (old_size - new_size) - (len - new_len));
...
FrameStatus checkWMBusFrame(std::vector<uchar> &data, size_t *frame_length,
                            ...
                            bool only_test) {
  // Nice clean:
  // 2A442D2C998734761B168D2021D0871921|58387802FF2071000413F81800004413F8180000615B
  // Ugly:
  // 00615B2A442D2C998734761B168D2021D0871921|58387802FF2071000413F81800004413F8180000615B
  ...
  *frame_length = payload_len + offset;
  if (data.size() < *frame_length) {
    ...
    if (only_test) {
      ...
      *payload_len_out = payload_len;
      *frame_length = payload_len + offset;
      warning("(wmbus) not enough bytes, frame length byte changed from "
              "%d(%02x) to %d(%02x)!\n",
              data[offset - 1], data[offset - 1], payload_len, payload_len);
...
std::string decodeTPLStatusByteOnlyStandardBits(uchar sts) {
  ...
  if (sts == 0)
    return "OK";
  if ((sts & 0x03) == 0x01)
    s += "BUSY "; // Meter busy, cannot respond.
  if ((sts & 0x03) == 0x02)
    s += "ERROR "; // E.g. meter failed to understand a message sent to it.
                   // More information about the error can be sent using error
                   // reporting, EN13757-3:2018 §10
  if ((sts & 0x03) == 0x03)
    s += "ALARM "; // E.g. an abnormal condition like water is continuously
                   // running.

  if ((sts & 0x04) == 0x04)
    s += "POWER_LOW "; // E.g. battery end of life or external power supply
                       // failure
  if ((sts & 0x08) == 0x08)
    s += "PERMANENT_ERROR "; // E.g. meter needs service to work again.
  if ((sts & 0x10) == 0x10)
    s += "TEMPORARY_ERROR ";
...
std::string decodeTPLStatusByteWithMfct(uchar sts, Translate::Lookup &lookup) {
  std::string s = decodeTPLStatusByteOnlyStandardBits(sts);
  std::string t = "OK";
...
bool is_command(std::string b, std::string *cmd) {
  // Check if CMD(.)
  if (b.length() < 6)
    return false;
  if (b.rfind("CMD(", 0) != 0)
...
const char *toString(FrameStatus fs) {
  switch (fs) {
  case FrameStatus::PartialFrame:
    return "PartialFrame";
  case FrameStatus::FullFrame:
    return "FullFrame";
  case FrameStatus::ErrorInFrame:
    return "ErrorInFrame";
  case FrameStatus::TextAndNotFrame:
