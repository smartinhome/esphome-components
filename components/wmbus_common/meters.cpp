...
std::vector<DriverInfo *> &allDrivers() { return *registered_drivers_list_; }

void removeDriver(const std::string &name) {
  for (auto i = registered_drivers_list_->begin();
       i != registered_drivers_list_->end(); i++) {
    if ((*i)->name().str() == name) {
...
bool forceRegisterDriver(std::function<void(DriverInfo &)> setup) {
  ...
  for (auto &d : di.detect()) {
    for (DriverInfo *p : allDrivers()) {
      ...
      if (foo) {
        error("Internal error: driver %s tried to register the same auto "
              "detect combo as driver %s alread has taken!\n",
  ...
  // This code is invoked from the static initializers of DriverInfos when
  // starting wmbusmeters. Thus we do not yet know if the user has supplied
  // --debug or similar setting. To debug this you have to uncomment the printf
  // below. fprintf(stderr, "(STATIC) added driver: %s\n", n.c_str());
  return true;
}
...
MeterCommonImplementation::MeterCommonImplementation(MeterInfo &mi,
                                                     DriverInfo &di)
    : type_(di.type()), driver_name_(di.name()), driver_info_(&di),
      bus_(mi.bus), name_(mi.name),
...
void MeterCommonImplementation::addExtraCalculatedField(std::string ecf) {
  ...
  if (existing != NULL) {
    if (!canConvert(unit, existing->displayUnit())) {
      warning("Warning! Cannot add the calculated field: %s since it would "
              "conflict with the already declared field %s for quantity %s.\n",
...
MeterCommonImplementation::meterExtraConstantFields() {
  return extra_constant_fields_;
}

DriverName MeterCommonImplementation::driverName() { return driver_name_; }

DriverInfo *MeterCommonImplementation::driverInfo() { return driver_info_; }

void MeterCommonImplementation::setMeterType(MeterType mt) { type_ = mt; }

void MeterCommonImplementation::addLinkMode(LinkMode lm) {
  link_modes_.addLinkMode(lm);
}

void MeterCommonImplementation::setMfctTPLStatusBits(
...
void MeterCommonImplementation::addNumericFieldWithExtractor(
    std::string vname, std::string help, PrintProperties print_properties,
    Quantity vquantity, VifScaling vif_scaling, DifSignedness dif_signedness,
    FieldMatcher matcher, Unit display_unit, double scale) {
  size_t index = num_driver_fields_++;
  field_infos_.emplace_back(FieldInfo(
      index, vname, vquantity,
      display_unit == Unit::Unknown ? defaultUnitForQuantity(vquantity)
                                    : display_unit,
      vif_scaling, dif_signedness, scale, matcher, help, print_properties, NULL,
      NULL, NULL, NULL, NoLookup, /* Lookup table */
      NULL,                       /* Formula */
      this                        /* Meter */
      ));
}

void MeterCommonImplementation::addNumericFieldWithCalculator(
    std::string vname, std::string help, PrintProperties print_properties,
    Quantity vquantity, std::string formula, Unit display_unit) {
  ...
  size_t index = num_driver_fields_++;
  field_infos_.push_back(FieldInfo(
      index, vname, vquantity,
      display_unit == Unit::Unknown ? defaultUnitForQuantity(vquantity)
                                    : display_unit,
      VifScaling::Auto, DifSignedness::Signed, 1.0, FieldMatcher::noMatcher(),
      help, print_properties, NULL, NULL, NULL, NULL,
...
void MeterCommonImplementation::addNumericFieldWithCalculatorAndMatcher(
    std::string vname, std::string help, PrintProperties print_properties,
    Quantity vquantity, std::string formula, FieldMatcher matcher,
    Unit display_unit) {
  Formula *f = newFormula();
  bool ok = f->parse(this, formula);
  if (!ok) {
  ...
  field_infos_.push_back(FieldInfo(
      ...
      display_unit == Unit::Unknown ? defaultUnitForQuantity(vquantity)
                                    : display_unit,
      VifScaling::Auto, DifSignedness::Signed, 1.0, matcher, help,
      print_properties, NULL, NULL, NULL, NULL, NoLookup, /* Lookup table */
      f,                                                  /* Formula */
      this                                                /* Meter */
      ));
}
...
void MeterCommonImplementation::addNumericField(
    std::string vname, Quantity vquantity, PrintProperties print_properties,
    std::string help, Unit display_unit) {
  size_t index = num_driver_fields_++;
  field_infos_.emplace_back(FieldInfo(
      index, vname, vquantity,
      display_unit == Unit::Unknown ? defaultUnitForQuantity(vquantity)
                                    : display_unit,
      VifScaling::None, DifSignedness::Signed, 1.0, FieldMatcher::noMatcher(),
      help, print_properties,
      NULL, // getValueFunc,
      NULL,
      NULL,           // setValueFunc
      NULL, NoLookup, /* Lookup table */
      NULL,           /* Formula */
      this            /* Meter */
      ));
}
...
void MeterCommonImplementation::addStringFieldWithExtractor(
  ...
  size_t index = num_driver_fields_++;
  field_infos_.emplace_back(FieldInfo(
      index, vname, Quantity::Text, defaultUnitForQuantity(Quantity::Text),
      VifScaling::None, DifSignedness::Signed, 1.0, matcher, help,
      print_properties, NULL, NULL, NULL, NULL, NoLookup, /* Lookup table */
      NULL,                                               /* Formula */
      this                                                /* Meter */
      ));
...
void MeterCommonImplementation::addStringFieldWithExtractorAndLookup(
    std::string vname, std::string help, PrintProperties print_properties,
    FieldMatcher matcher, Translate::Lookup lookup) {
  size_t index = num_driver_fields_++;
  field_infos_.emplace_back(FieldInfo(
      index, vname, Quantity::Text, defaultUnitForQuantity(Quantity::Text),
      VifScaling::None, DifSignedness::Signed, 1.0, matcher, help,
      print_properties, NULL, NULL, NULL, NULL, lookup, NULL, /* Formula */
      this                                                    /* Meter */
      ));
}

void MeterCommonImplementation::addStringField(
    std::string vname, std::string help, PrintProperties print_properties) {
  size_t index = num_driver_fields_++;
  field_infos_.emplace_back(FieldInfo(
      index, vname, Quantity::Text, defaultUnitForQuantity(Quantity::Text),
      VifScaling::None, DifSignedness::Signed, 1.0, FieldMatcher(), help,
      print_properties, NULL, NULL, NULL, NULL, NoLookup, /* Lookup table */
      NULL,                                               /* Formula */
      this                                                /* Meter */
      ));
...
std::vector<AddressExpression> &
MeterCommonImplementation::addressExpressions() {
...
std::string MeterCommonImplementation::name() { return name_; }

int MeterCommonImplementation::numUpdates() { return num_updates_; }
...
std::string MeterCommonImplementation::datetimeOfUpdateRobot() {
  char datetime[40];
  memset(datetime, 0, sizeof(datetime));
  // This is the date time in the Greenwich timezone (Zulu time), dont get
  // surprised!
...
time_t MeterCommonImplementation::pollInterval() { return poll_interval_; }

bool MeterCommonImplementation::usesPolling() {
  return link_modes_.has(LinkMode::MBUS) || link_modes_.has(LinkMode::C2) ||
         link_modes_.has(LinkMode::T2) || link_modes_.has(LinkMode::S2);
}
...
bool driverNeedsPolling(DriverName &dn) {
  ...
  return di->linkModes().has(LinkMode::MBUS) ||
         ...
         di->linkModes().has(LinkMode::T2) || di->linkModes().has(LinkMode::S2);
}

const char *toString(MeterType type) {
#define X(tname)                                                               \
  if (type == MeterType::tname)                                                \
    return #tname;
  LIST_OF_METER_TYPES
#undef X
  return "unknown";
...
#define X(tname)                                                               \
  if (type == #tname)                                                          \
    return MeterType::tname;
  LIST_OF_METER_TYPES
#undef X
  return MeterType::UnknownMeter;
}

std::string toString(DriverInfo &di) { return di.name().str(); }

bool MeterCommonImplementation::isTelegramForMeter(Telegram *t, Meter *meter,
                                                   MeterInfo *mi) {
  std::string name;
  std::vector<AddressExpression> address_expressions;
  std::string driver_name;

  assert((meter && !mi) || (!meter && mi));
  ...
  bool valid_driver =
      isMeterDriverValid(driver_name, t->dll_mfct, t->dll_type, t->dll_version);
  if (!valid_driver && t->tpl_id_found) {
    valid_driver = isMeterDriverValid(driver_name, t->tpl_mfct, t->tpl_type,
                                      t->tpl_version);
  }
...
MeterKeys *MeterCommonImplementation::meterKeys() { return &meter_keys_; }

int MeterCommonImplementation::index() { return index_; }

void MeterCommonImplementation::setIndex(int i) { index_ = i; }

std::string MeterCommonImplementation::bus() { return bus_; }
...
void MeterCommonImplementation::triggerUpdate(Telegram *t) {
  ...
  datetime_of_update_ =
      t->about.timestamp ? t->about.timestamp : datetime_of_poll_;
  num_updates_++;
  t->handled = true;
}
...
std::string build_id(Address &a, IdentityMode im) {
  std::string id = a.id;
  if (im == IdentityMode::ID_MFCT || im == IdentityMode::FULL) {
...
bool checkCommonField(std::string *buf, std::string desired_field, Meter *m,
                      Telegram *t, char c, bool human_readable) {
...
bool checkPrintableField(std::string *buf, std::string desired_field, Meter *m,
                         Telegram *t, char c, std::vector<FieldInfo> &fields,
                         bool human_readable) {
    ...
    if (fi.xuantity() == Quantity::Text) {
      ...
      if (fi.displayUnit() == Unit::DateLT) {
        ...
        *buf += c;
        ...
        // Default unit.
        *buf += valueToString(m->getNumericValue(&fi, fi.displayUnit()),
                              fi.displayUnit());
        if (human_readable) {
          *buf += " ";
...
bool checkConstantField(std::string *buf, std::string field, char c,
                        std::vector<std::string> *extra_constant_fields) {
  // Ok, lets look for extra constant fields and print any such static
  // information.
...
std::string concatFields(Meter *m, Telegram *t, char c,
                         std::vector<FieldInfo> &prints, bool human_readable,
...
void MeterCommonImplementation::processFieldExtractors(Telegram *t) {
  ...
  sort(sorted_entries.begin(), sorted_entries.end(),
       [](const DVEntry *a, const DVEntry *b) -> bool {
  ...
  for (FieldInfo &fi : field_infos_) {
    ...
    debug("(meters) trying field info %s(%s)[%d]...\n", fi.vname().c_str(),
          toString(fi.xuantity()), fi.index());

    // Iterate through dv_entries in the telegram in the same order the telegram
    // presented them.
    for (DVEntry *dve : sorted_entries) {
      if (fi.hasMatcher() && fi.matches(dve)) {
        ...
        if (fi.matcher().index_nr != IndexNr(current_match_nr) &&
        ...
        } else if (founds[&fi].count(dve) == 0 ||
                   fi.matcher().expectedToMatchAgainstMultipleEntries()) {
          debug("(meters) using field info %s(%s)[%d] to extract %s at offset "
                "%d\n",
                fi.vname().c_str(), toString(fi.xuantity()), fi.index(),
                dve->dif_vif_key.str().c_str(), dve->offset);
          ...
          verbose("(meter) while processing field extractors ignoring dventry "
                  "%s at offset %d matching since "
                  "field %s was already matched against offsets %s !\n",
                  dve->dif_vif_key.str().c_str(), dve->offset,
                  fi.vname().c_str(), olds.c_str());
        }
      }
    }
  }
  ...
  for (FieldInfo &fi : field_infos_) {
    ...
    } else if (founds.count(&fi) == 0 &&
               fi.printProperties().hasINCLUDETPLSTATUS()) {
...
std::string MeterCommonImplementation::getStatusField(FieldInfo *fi) {
  ...
  if (string_values_.count(field_name_no_unit) == 0) {
    return "null"; // This is translated to a real(non-std::string) null in the
                   // json.
  }
  ...
  // Sort all found flags and remove any duplicates. A well designed meter
  // decoder should not be able to generate duplicates.
...
void MeterCommonImplementation::setNumericValue(FieldInfo *fi, DVEntry *dve,
                                                Unit u, double v) {
  std::string field_name_no_unit;

  if (dve == NULL) {
    std::string field_name_no_unit = fi->vname();
    numeric_values_[std::pair<std::string, Unit>(
        field_name_no_unit, fi->displayUnit())] = NumericField(u, v, fi);
  } else {
    field_name_no_unit = fi->generateFieldNameNoUnit(this, dve);
    numeric_values_[std::pair<std::string, Unit>(
        field_name_no_unit, fi->displayUnit())] = NumericField(u, v, fi, *dve);
  }
}

void MeterCommonImplementation::setNumericValue(std::string vname, Unit u,
                                                double v) {
  Quantity q = toQuantity(u);
  FieldInfo *fi = findFieldInfo(vname, q);

  if (fi == NULL) {
    warning("(meter) cannot set numeric value %g %s for non-existant field "
            "\"%s\" %s\n",
...
double MeterCommonImplementation::getNumericValue(FieldInfo *fi, Unit to) {
  ...
  if (numeric_values_.count(key) == 0) {
    return std::numeric_limits<double>::quiet_NaN(); // This is translated into
                                                     // a null in the json.
  }
...
void MeterCommonImplementation::setStringValue(FieldInfo *fi, std::string v,
                                               DVEntry *dve) {
  std::string field_name_no_unit;
...
void MeterCommonImplementation::setStringValue(std::string vname, std::string v,
                                               DVEntry *dve) {
  FieldInfo *fi = findFieldInfo(vname, Quantity::Text);

  if (fi == NULL) {
    warning("(meter) cannot set std::string value %s for non-existant field "
            "\"%s\"\n",
            v.c_str(), vname.c_str());
    return;
  }
...
FieldInfo *MeterCommonImplementation::findFieldInfo(std::string vname,
                                                    Quantity xuantity) {
  FieldInfo *found = NULL;
  for (FieldInfo &p : field_infos_) {
    if (p.vname() == vname && p.xuantity() == xuantity) {
      found = &p;
      break;
    }
  }
...
std::string
MeterCommonImplementation::renderJsonOnlyDefaultUnit(std::string vname,
                                                     Quantity xuantity) {
  FieldInfo *fi = findFieldInfo(vname, xuantity);

  if (fi == NULL)
    return "unknown field " + vname;
...
FieldInfo::FieldInfo(
    int index, std::string vname, Quantity xuantity, Unit display_unit,
    VifScaling vif_scaling, DifSignedness dif_signedness, double scale,
    FieldMatcher matcher, std::string help, PrintProperties print_properties,
    ...
    Translate::Lookup lookup, Formula *formula, Meter *m)
    : index_(index), vname_(vname), xuantity_(xuantity),
      display_unit_(display_unit), vif_scaling_(vif_scaling),
      dif_signedness_(dif_signedness), scale_(scale), matcher_(matcher),
      help_(help), print_properties_(print_properties),
      ...
      set_string_value_override_(set_string_value_override), lookup_(lookup),
      formula_(formula), field_name_(newStringInterpolator()),
...
std::string FieldInfo::renderJson(Meter *m, DVEntry *dve) {
  ...
  if (xuantity() == Quantity::Text) {
    ...
    if (v == "null") {
      // Yes, right now a meter cannot send a string value "something":"null" it
      // will be translated into "something":null in the json, indicating that
      // there is no value. This should not be a problem for now. Lets deal with
      // it when a meter decides to send "null" as its version string for
      // example.
      s += "\"" + field_name + "\":null";
    } else {
      // Normally the string values are quoted in json. TODO quote the value
      // properly. A well crafted meter could send a version string with " and
      // break the json format.
      s += "\"" + field_name + "\":\"" + v + "\"";
    }
  } else {
    if (displayUnit() == Unit::DateLT) {
      s += "\"" + field_name + "_" + display_unit_s + "\":\"" +
           strdate(m->getNumericValue(field_name, Unit::DateLT)) + "\"";
    } else if (displayUnit() == Unit::DateTimeLT) {
      s += "\"" + field_name + "_" + display_unit_s + "\":\"" +
           strdatetime(m->getNumericValue(field_name, Unit::DateTimeLT)) + "\"";
    } else if (displayUnit() == Unit::DateTimeUTC) {
      s += "\"" + field_name + "_" + display_unit_s + "\":\"" +
           strTimestampUTC(m->getNumericValue(field_name, Unit::DateTimeUTC)) +
           "\"";
    } else {
      // All numeric values.
      s += "\"" + field_name + "_" + display_unit_s + "\":" +
           valueToString(m->getNumericValue(field_name, displayUnit()),
                         displayUnit());
    }
  }
...
void MeterCommonImplementation::printMeter(
  ...
  if (human_readable)
    *human_readable = concatFields(this, t, '\t', field_infos_, true,
                                   selected_fields, extra_constant_fields);
  if (fields)
    *fields = concatFields(this, t, separator, field_infos_, false,
                           selected_fields, extra_constant_fields);
  ...
  if (json) {
    ...
    std::map<FieldInfo *, std::set<DVEntry *>>
        founds; // Multiple dventries can match to a single field info.
    std::set<std::string> found_vnames;

    for (auto &p : numeric_values_) {
      std::string vname = p.first.first;
      NumericField &nf = p.second;
      if (nf.field_info->printProperties().hasHIDE())
        continue;
    ...
    for (auto &p : string_values_) {
      ...
      if (sf.field_info->printProperties().hasHIDE())
        continue;
...
void MeterCommonImplementation::setExpectedELLSecurityMode(
    ELLSecurityMode dsm) {
...
bool is_driver_and_extras(const std::string &t, DriverName *out_driver_name,
                          std::string *out_extras) {
  // piigth(jump=foo)
  // multical21
  DriverInfo di;
  size_t ps = t.find('(');
...
bool MeterInfo::parse(std::string n, std::string d, std::string aes,
                      std::string k) {
  clear();
  ...
  for (auto &p : parts) {
    if (!driverextras_checked &&
        is_driver_and_extras(p, &driver_name, &extras)) {
...
bool MeterInfo::usesPolling() {
  return link_modes.has(LinkMode::MBUS) || link_modes.has(LinkMode::C2) ||
         link_modes.has(LinkMode::T2) || link_modes.has(LinkMode::S2);
}
...
bool isValidKey(const std::string &key, MeterInfo &mi) {
  ...
  if (mi.driver_name.str() == "izar" || mi.driver_name.str() == "hydrus") {
...
bool FieldInfo::hasMatcher() { return matcher_.active == true; }

bool FieldInfo::hasFormula() { return formula_ != NULL; }

bool FieldInfo::matches(DVEntry *dve) { return matcher_.matches(*dve); }

std::string FieldInfo::str() {
  return tostrprintf("%d %s_%s (%s) %s [%s] \"%s\"", index_, vname_.c_str(),
                     unitToStringLowerCase(display_unit_).c_str(),
                     toString(xuantity_), toString(vif_scaling_),
                     matcher_.str().c_str(), help_.c_str());
}

DriverName MeterInfo::driverName() { return driver_name; }
...
bool FieldInfo::extractNumeric(Meter *m, Telegram *t, DVEntry *dve) {
  ...
  if (dve == NULL) {
    if (key == "") {
      // Search for key.
      bool ok =
          findKeyWithNr(matcher_.measurement_type, matcher_.vif_range,
                        matcher_.storage_nr_from.intValue(),
                        matcher_.tariff_nr_from.intValue(),
                        matcher_.index_nr.intValue(), &key, &t->dv_entries);
      // No entry was found.
      if (!ok)
        return false;
    }
  ...
  if (dve->extractDouble(&extracted_double_value, auto_vif_scaling,
                         force_unsigned)) {
    ...
    debug("(meter) %s %s decoded %s default %s value %g (scale %g)\n",
          toString(matcher_.vif_range), field_name.c_str(),
          unitToStringLowerCase(decoded_unit).c_str(),
          unitToStringLowerCase(display_unit_).c_str(), extracted_double_value,
          scale());

    if (scale() != 1.0) {
      // Hardcoded scale factor for this field used for manufacturer specific
      // values without vif units.
      extracted_double_value *= scale();
    }
    if (overrideConversion(decoded_unit, display_unit_)) {
      // Special case! Transform the decoded unit into the display unit. I.e.
      // kwh was replaced with kvarh.
      decoded_unit = display_unit_;
    }
    m->setNumericValue(
        this, dve, display_unit_,
        convert(extracted_double_value, decoded_unit, display_unit_));
...
static std::string add_tpl_status(std::string existing_status, Meter *m,
                                  Telegram *t) {
...
bool FieldInfo::extractString(Meter *m, Telegram *t, DVEntry *dve) {
  ...
  if (lookup_.hasLookups() || (print_properties_.hasINCLUDETPLSTATUS())) {
    ...
    error("Internal error: Cannot extract text std::string from vif %s in "
          "%s:%d\n",
          toString(matcher_.vif_range), __FILE__, __LINE__);
  }
  return found;
}
...
bool checkFieldsEmpty(std::set<std::string> &fields, std::string driver_name) {
  if (fields.size() > 0) {
    ...
    warning("(meter) when adding common fields to driver %s, these fields were "
            "not found: %s\n",
            driver_name.c_str(), info.c_str());
    return false;
  }
  return true;
}

bool MeterCommonImplementation::addOptionalLibraryFields(
  ...
  if (checkIf(fields, "actuality_duration_s")) {
    addNumericFieldWithExtractor(
        ...
        DEFAULT_PRINT_PROPERTIES, Quantity::Time, VifScaling::Auto,
  ...
  if (checkIf(fields, "fabrication_no")) {
    addStringFieldWithExtractor("fabrication_no", "Fabrication number.",
  ...
  if (checkIf(fields, "enhanced_id")) {
    addStringFieldWithExtractor("enhanced_id",
  ...
  if (checkIf(fields, "software_version")) {
    addStringFieldWithExtractor("software_version", "Software version.",
                                DEFAULT_PRINT_PROPERTIES,
                                FieldMatcher::build()
                                    .set(MeasurementType::Instantaneous)
  ...
  if (checkIf(fields, "manufacturer")) {
    addStringFieldWithExtractor("manufacturer", "Meter manufacturer.",
                                DEFAULT_PRINT_PROPERTIES,
                                FieldMatcher::build()
                                    .set(MeasurementType::Instantaneous)
  ...
  if (checkIf(fields, "model_version")) {
    addStringFieldWithExtractor("model_version", "Meter model version.",
                                DEFAULT_PRINT_PROPERTIES,
                                FieldMatcher::build()
                                    .set(MeasurementType::Instantaneous)
  ...
  if (checkIf(fields, "firmware_version")) {
    addStringFieldWithExtractor("firmware_version", "Meter firmware version.",
                                DEFAULT_PRINT_PROPERTIES,
                                FieldMatcher::build()
                                    .set(MeasurementType::Instantaneous)
  ...
  if (checkIf(fields, "parameter_set")) {
    addStringFieldWithExtractor("parameter_set",
  ...
  if (checkIf(fields, "customer")) {
    addStringFieldWithExtractor("customer", "Customer name.",
                                DEFAULT_PRINT_PROPERTIES,
                                FieldMatcher::build()
                                    .set(MeasurementType::Instantaneous)
  ...
  if (checkIf(fields, "location")) {
    addStringFieldWithExtractor("location",
  ...
  if (checkIf(fields, "operating_time_h")) {
    addNumericFieldWithExtractor("operating_time",
                                 "How long the meter has been collecting data.",
                                 DEFAULT_PRINT_PROPERTIES, Quantity::Time,
  ...
  if (checkIf(fields, "on_time_h")) {
    addNumericFieldWithExtractor("on_time",
  ...
  if (checkIf(fields, "meter_date")) {
    addStringFieldWithExtractor("meter_date",
  ...
  if (checkIf(fields, "meter_date_at_error")) {
    addStringFieldWithExtractor("meter_date_at_error",
  ...
  if (checkIf(fields, "meter_datetime")) {
    addStringFieldWithExtractor(
        "meter_datetime", "Date and time when the meter sent the telegram.",
        DEFAULT_PRINT_PROPERTIES,
        FieldMatcher::build()
            .set(MeasurementType::Instantaneous)
  ...
  if (checkIf(fields, "meter_datetime_at_error")) {
    addStringFieldWithExtractor("meter_datetime_at_error",
  ...
  if (checkIf(fields, "total_m3")) {
    addNumericFieldWithExtractor(
        "total", "The total media volume consumption recorded by this meter.",
  ...
  if (checkIf(fields, "target_m3")) {
    addNumericFieldWithExtractor(
        "target", "The volume recorded by this meter at the target date.",
        DEFAULT_PRINT_PROPERTIES, Quantity::Volume, VifScaling::Auto,
        DifSignedness::Signed,
        FieldMatcher::build()
            .set(MeasurementType::Instantaneous)
  ...
  if (checkIf(fields, "target_date")) {
    addNumericFieldWithExtractor(
        ...
        DEFAULT_PRINT_PROPERTIES, Quantity::PointInTime, VifScaling::Auto,
        DifSignedness::Signed,
        FieldMatcher::build()
            .set(MeasurementType::Instantaneous)
            .set(VIFRange::Date)
  ...
  if (checkIf(fields, "total_forward_m3")) {
    addNumericFieldWithExtractor("total_forward",
                                 "The total media volume flowing forward.",
                                 DEFAULT_PRINT_PROPERTIES, Quantity::Volume,
                                 VifScaling::Auto, DifSignedness::Signed,
                                 FieldMatcher::build()
                                     .set(MeasurementType::Instantaneous)
                                     .set(VIFRange::Volume)
  ...
  if (checkIf(fields, "total_backward_m3")) {
    addNumericFieldWithExtractor("total_backward",
  ...
  if (checkIf(fields, "flow_temperature_c")) {
    addNumericFieldWithExtractor(
        "flow_temperature", "Forward media temperature.",
        DEFAULT_PRINT_PROPERTIES, Quantity::Temperature, VifScaling::Auto,
        DifSignedness::Signed,
        FieldMatcher::build()
            .set(MeasurementType::Instantaneous)
  ...
  if (checkIf(fields, "external_temperature_c")) {
    addNumericFieldWithExtractor(
        "external_temperature", "Temperature outside of meter.",
        DEFAULT_PRINT_PROPERTIES, Quantity::Temperature, VifScaling::Auto,
        DifSignedness::Signed,
        FieldMatcher::build()
            .set(MeasurementType::Instantaneous)
  ...
  if (checkIf(fields, "return_temperature_c")) {
    addNumericFieldWithExtractor(
        "return_temperature", "Return media temperature.",
        DEFAULT_PRINT_PROPERTIES, Quantity::Temperature, VifScaling::Auto,
        DifSignedness::Signed,
        FieldMatcher::build()
            .set(MeasurementType::Instantaneous)
  ...
  if (checkIf(fields, "volume_flow_m3h")) {
    addNumericFieldWithExtractor("volume_flow", "Media volume flow.",
                                 DEFAULT_PRINT_PROPERTIES, Quantity::Flow,
                                 VifScaling::Auto, DifSignedness::Signed,
                                 FieldMatcher::build()
                                     .set(MeasurementType::Instantaneous)
  ...
  if (checkIf(fields, "access_counter")) {
    addNumericFieldWithExtractor(
        "access", "Meter access counter.", DEFAULT_PRINT_PROPERTIES,
        Quantity::Dimensionless, VifScaling::None, DifSignedness::Unsigned,
        FieldMatcher::build()
            .set(MeasurementType::Instantaneous)
  ...
  if (checkIf(fields, "consumption_hca")) {
    addNumericFieldWithExtractor(
        "consumption", "The current heat cost allocation for this meter.",
        DEFAULT_PRINT_PROPERTIES, Quantity::HCA, VifScaling::Auto,
        DifSignedness::Signed,
        FieldMatcher::build()
            .set(MeasurementType::Instantaneous)
...
const char *toString(VifScaling s) {
  switch (s) {
  case VifScaling::Auto:
    return "Auto";
  case VifScaling::None:
    return "None";
  case VifScaling::Unknown:
    return "Unknown";
  }
  return "?";
}
...
const char *toString(DifSignedness s) {
  switch (s) {
  case DifSignedness::Signed:
    return "Signed";
  case DifSignedness::Unsigned:
    return "Unsigned";
  case DifSignedness::Unknown:
    return "Unknown";
  }
  return "?";
}
...
const char *toString(PrintProperty p) {
  ...
  case PrintProperty::INJECT_INTO_STATUS:
    return "INJECT_INTO_STATUS";
  case PrintProperty::HIDE:
    return "HIDE";
  case PrintProperty::Unknown:
    return "Unknown";
  }
...
PrintProperty toPrintProperty(const char *s) {
  ...
  if (!strcmp(s, "INJECT_INTO_STATUS"))
    return PrintProperty::INJECT_INTO_STATUS;
  if (!strcmp(s, "HIDE"))
    return PrintProperty::HIDE;
  if (!strcmp(s, "Unknown"))
