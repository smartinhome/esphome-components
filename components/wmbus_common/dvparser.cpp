...
const char *toString(VIFRange v) {
  ...
  case VIFRange::Any:
    return "Any";
#define X(name, from, to, quantity, unit)                                      \
  case VIFRange::name:                                                         \
    return #name;
    LIST_OF_VIF_RANGES
#undef X
...
VIFCombinable toVIFCombinable(const char *s) {
  ...
  if (!strcmp(s, "Any"))
    return VIFCombinable::Any;
#define X(name, from, to)                                                      \
  if (!strcmp(s, #name))                                                       \
    return VIFCombinable::name;
...
const char *toString(VIFCombinable v) {
  switch (v) {
  case VIFCombinable::None:
    return "None";
  case VIFCombinable::Any:
    return "Any";
#define X(name, from, to)                                                      \
  case VIFCombinable::name:                                                    \
    return #name;
    LIST_OF_VIF_COMBINABLES
#undef X
  }
...
#define X(name, from, to, quantity, unit)                                      \
  {                                                                            \
    if (from <= v.intValue() && v.intValue() <= to)                            \
      return unit;                                                             \
  }
  LIST_OF_VIF_RANGES
#undef X
...
bool isInsideVIFRange(Vif vif, VIFRange vif_range) {
  ...
  if (vif_range == VIFRange::AnyEnergyVIF) {
    return isInsideVIFRange(vif, VIFRange::EnergyWh) ||
...
#define X(name, from, to, quantity, unit)                                      \
  if (VIFRange::name == vif_range) {                                           \
    return from <= vif.intValue() && vif.intValue() <= to;                     \
  }
  LIST_OF_VIF_RANGES
#undef X
  return false;
...
bool parseDV(Telegram *t, std::vector<uchar> &databytes,
  ...
  // Or if the dif says variable length, then the first data byte specifies the
  // number of data bytes.

  // DIF again...

  // A Dif(Difes)Vif(Vifes) identifier can be for example be the 02FF20 for the
  // Multical21 vendor specific status bits. The parser then uses this
  // identifier as a key to store the data bytes in a map. The same identifier
  // could occur several times in a telegram, even though it often don't. Since
  // the first occurence is stored under 02FF20, the second identical identifier
  // stores its data under the key "02FF20_2" etc for 3 and forth... A proper
  // meter would use storagenr etc to differentiate between different
  ...
  for (;;) {
    ...
    if (force_mfct_index != -1) {
      ...
      if (index >= force_mfct_index) {
        DEBUG_PARSER(
            "(dvparser) manufacturer specific data, parsing is done.\n", dif);
        size_t datalen = std::distance(data, data_end);
        std::string value = bin2hex(data, data_end, datalen);
        t->addExplanationAndIncrementPos(
            data, datalen, KindOfData::CONTENT, Understanding::NONE,
            "manufacturer specific data %s", value.c_str());
        break;
      }
    }
    ...
    DEBUG_PARSER("(dvparser debug) dif=%02x datalen=%d \"%s\" type=%s\n", dif,
                 datalen, difType(dif).c_str(),
                 measurementTypeName(mt).c_str());

    if (datalen == -2) {
      if (dif == 0x0f) {
        DEBUG_PARSER("(dvparser) reached dif %02x manufacturer specific data, "
                     "parsing is done.\n",
                     dif);
        ...
        t->addExplanationAndIncrementPos(
            data, datalen, KindOfData::CONTENT, Understanding::NONE,
            "%02X manufacturer specific data %s", dif, value.c_str());
        break;
      }
      if (dif == 0x1f) {
        DEBUG_PARSER(
            "(dvparser) reached dif %02x more records in next telegram.\n",
            dif);
        datalen = std::distance(data, data_end);
        std::string value = bin2hex(data + 1, data_end, datalen - 1);
        t->mfct_0f_index = 1 + std::distance(data_start, data);
        ...
        assert(t->mfct_0f_index >= 0);
        t->addExplanationAndIncrementPos(
            data, datalen, KindOfData::CONTENT, Understanding::FULL,
            "%02X more data in next telegram %s", dif, value.c_str());
        break;
      }
      DEBUG_PARSER("(dvparser) reached unknown dif %02x treating remaining "
                   "data as manufacturer specific, parsing is done.\n",
                   dif);
      datalen = std::distance(data, data_end);
      std::string value = bin2hex(data + 1, data_end, datalen - 1);
      t->mfct_0f_index = 1 + std::distance(data_start, data);
      ...
      t->addExplanationAndIncrementPos(
          data, datalen, KindOfData::CONTENT, Understanding::NONE,
          "%02X unknown dif treating remaining data as mfct specific %s", dif,
          value.c_str());
      break;
    }
    if (dif == 0x2f) {
      t->addExplanationAndIncrementPos(*format, 1, KindOfData::PROTOCOL,
                                       Understanding::FULL, "%02X skip", dif);
      DEBUG_PARSER("\n");
      continue;
    }
    if (datalen == -1) {
    ...
    while (has_another_dife) {
      ...
      DEBUG_PARSER(
          "(dvparser debug) dife=%02x (subunit=%d tariff=%d storagenr=%d)\n",
      ...
      if (data_has_difvifs) {
        ...
        t->addExplanationAndIncrementPos(
            *format, 1, KindOfData::PROTOCOL, Understanding::FULL,
            "%02X dife (subunit=%d tariff=%d storagenr=%d)", dife, subunit,
            tariff, storage_nr);
      } else {
        id_bytes.push_back(**format);
        (*format)++;
    ...
    // Check if this is marker for one of the extended sets of vifs: first,
    // second and third or manufacturer.
    ...
    // One meter (qsmoke) does not set the high bit, since the next byte is not
    // strictly a vife, it is a length byte. Another meter does set the hight
    // bit.... We accept both hear, I do not yet know what the standard says.
    if (vif == 0x7c || vif == 0xfc) {
      ...
      t->addExplanationAndIncrementPos(*format, 1, KindOfData::PROTOCOL,
                                       Understanding::FULL, "%02X viflen (%d)",
                                       viflen, viflen);
      for (uchar i = 0; i < viflen; ++i) {
        ...
        uchar v = **format;
        t->addExplanationAndIncrementPos(*format, 1, KindOfData::PROTOCOL,
                                         Understanding::FULL, "%02X vif (%c)",
                                         v, v);
        id_bytes.push_back(v);
      }
    }

    // Do we have another vife byte? We better have one, if extension_vif is
    // true.
    ...
    if (variable_length) {
      ...
      datalen = *(data);
      t->addExplanationAndIncrementPos(data, 1, KindOfData::PROTOCOL,
                                       Understanding::FULL, "%02X varlen=%d",
                                       *(data + 0), datalen);
    ...
    (*dv_entries)[key] = {
        offset, DVEntry(offset, key, mt, Vif(full_vif), found_combinable_vifs,
                        found_combinable_vifs_raw, StorageNr(storage_nr),
                        TariffNr(tariff), SubUnitNr(subunit), value)};
    ...
    if (value.length() > 0) {
      ...
      t->addExplanationAndIncrementPos(data, datalen, KindOfData::CONTENT,
                                       Understanding::NONE, "%s",
                                       value.c_str());
  ...
  uint16_t hash =
      crc16_EN13757(safeButUnsafeVectorPtr(format_bytes), format_bytes.size());
...
void extractDV(DifVifKey &dvk, uchar *dif, int *vif, bool *has_difes,
               bool *has_vifes) {
  std::string tmp = dvk.str();
  extractDV(tmp, dif, vif, has_difes, has_vifes);
}

void extractDV(std::string &s, uchar *dif, int *vif, bool *has_difes,
               bool *has_vifes) {
...
bool extractDVuint32(DVEntryMap *dv_entries,
  ...
  *value = (uint32_t(v[3]) << 24) | (uint32_t(v[2]) << 16) |
           (uint32_t(v[1]) << 8) | uint32_t(v[0]);
  return true;
}
...
bool extractDVdouble(DVEntryMap *dv_entries,
  ...
  if ((*dv_entries).count(key) == 0) {
    verbose("(dvparser) warning: cannot extract double from non-existant key "
            "\"%s\"\n",
            key.c_str());
    *offset = 0;
    *value = 0;
...
bool checkSizeHex(size_t expected_len, DifVifKey &dvk, std::string &v) {
  ...
  warning("(dvparser) bad decode since difvif %s expected %d hex chars but got "
          "\"%s\"\n",
...
bool DVEntry::extractDouble(double *out, bool auto_scale, bool force_unsigned) {
  ...
  if (t == 0x0 || t == 0x8 || t == 0xd || t == 0xf) {
    // Cannot extract from nothing, selection for readout, variable length or
    // special. Variable length is used for compact varlen history. Should be
    // added in the future.
    return false;
  } else if (t == 0x1 || // 8 Bit Integer/Binary
             ...
             t == 0x7)   // 64 Bit Integer/Binary
  {
    ...
    if (t == 0x1) {
      if (!checkSizeHex(2, dif_vif_key, value))
        return false;
      assert(v.size() == 1);
      raw = v[0];
      if (!force_unsigned && (raw & (uint64_t)0x80UL) != 0) {
        negate = true;
        negate_mask = ~((uint64_t)0) << 8;
      }
    } else if (t == 0x2) {
      if (!checkSizeHex(4, dif_vif_key, value))
        return false;
      assert(v.size() == 2);
      raw = v[1] * 256 + v[0];
      if (!force_unsigned && (raw & (uint64_t)0x8000UL) != 0) {
        negate = true;
        negate_mask = ~((uint64_t)0) << 16;
      }
    } else if (t == 0x3) {
      if (!checkSizeHex(6, dif_vif_key, value))
        return false;
      assert(v.size() == 3);
      raw = v[2] * 256 * 256 + v[1] * 256 + v[0];
      if (!force_unsigned && (raw & (uint64_t)0x800000UL) != 0) {
        negate = true;
        negate_mask = ~((uint64_t)0) << 24;
      }
    } else if (t == 0x4) {
      if (!checkSizeHex(8, dif_vif_key, value))
        return false;
      assert(v.size() == 4);
      raw = ((unsigned int)v[3]) * 256 * 256 * 256 +
            ((unsigned int)v[2]) * 256 * 256 + ((unsigned int)v[1]) * 256 +
            ((unsigned int)v[0]);
      if (!force_unsigned && (raw & (uint64_t)0x80000000UL) != 0) {
        negate = true;
        negate_mask = ~((uint64_t)0) << 32;
      }
    } else if (t == 0x6) {
      if (!checkSizeHex(12, dif_vif_key, value))
        return false;
      assert(v.size() == 6);
      raw = ((uint64_t)v[5]) * 256 * 256 * 256 * 256 * 256 +
            ((uint64_t)v[4]) * 256 * 256 * 256 * 256 +
            ((uint64_t)v[3]) * 256 * 256 * 256 + ((uint64_t)v[2]) * 256 * 256 +
            ((uint64_t)v[1]) * 256 + ((uint64_t)v[0]);
      if (!force_unsigned && (raw & (uint64_t)0x800000000000UL) != 0) {
        negate = true;
        negate_mask = ~((uint64_t)0) << 48;
      }
    } else if (t == 0x7) {
      if (!checkSizeHex(16, dif_vif_key, value))
        return false;
      assert(v.size() == 8);
      raw = ((uint64_t)v[7]) * 256 * 256 * 256 * 256 * 256 * 256 * 256 +
            ...
            ((uint64_t)v[3]) * 256 * 256 * 256 + ((uint64_t)v[2]) * 256 * 256 +
            ((uint64_t)v[1]) * 256 + ((uint64_t)v[0]);
      if (!force_unsigned && (raw & (uint64_t)0x8000000000000000UL) != 0) {
        negate = true;
        negate_mask = 0;
      }
    }
    ...
    if (auto_scale)
      scale = vifScale(dif_vif_key.vif());
    *out = (draw) / scale;
  } else if (t == 0x9 || // 2 digit BCD
             t == 0xA || // 4 digit BCD
             t == 0xB || // 6 digit BCD
             t == 0xC || // 8 digit BCD
             t == 0xE)   // 12 digit BCD
  {
    // Negative BCD values are always visible in bcd. I.e. they are always
    // signed. Ignore assumption on signedness. 74140000 -> 00001474
    std::string &v = value;
    uint64_t raw = 0;
    bool negate = false;
    ...
    if (t == 0x9) {
      if (!checkSizeHex(2, dif_vif_key, v))
        return false;
      if (v[0] == 'F') {
        negate = true;
        v[0] = '0';
      ...
      if (!checkSizeHex(4, dif_vif_key, v))
        return false;
      if (v[2] == 'F') {
        negate = true;
        v[2] = '0';
      ...
      if (!checkSizeHex(6, dif_vif_key, v))
        return false;
      if (v[4] == 'F') {
        negate = true;
        v[4] = '0';
      }
      raw = (v[4] - '0') * 10 * 10 * 10 * 10 * 10 +
            (v[5] - '0') * 10 * 10 * 10 * 10 + (v[2] - '0') * 10 * 10 * 10 +
            (v[3] - '0') * 10 * 10 + (v[0] - '0') * 10 + (v[1] - '0');
    } else if (t == 0xC) {
      if (!checkSizeHex(8, dif_vif_key, v))
        return false;
      if (v[6] == 'F') {
        negate = true;
        v[6] = '0';
      }
      raw = (v[6] - '0') * 10 * 10 * 10 * 10 * 10 * 10 * 10 +
            (v[7] - '0') * 10 * 10 * 10 * 10 * 10 * 10 +
            (v[4] - '0') * 10 * 10 * 10 * 10 * 10 +
            (v[5] - '0') * 10 * 10 * 10 * 10 + (v[2] - '0') * 10 * 10 * 10 +
            (v[3] - '0') * 10 * 10 + (v[0] - '0') * 10 + (v[1] - '0');
    } else if (t == 0xE) {
      if (!checkSizeHex(12, dif_vif_key, v))
        return false;
      if (v[10] == 'F') {
        negate = true;
        v[10] = '0';
      ...
      raw =
          (v[10] - '0') * 10 * 10 * 10 * 10 * 10 * 10 * 10 * 10 * 10 * 10 * 10 +
          (v[11] - '0') * 10 * 10 * 10 * 10 * 10 * 10 * 10 * 10 * 10 * 10 +
          (v[8] - '0') * 10 * 10 * 10 * 10 * 10 * 10 * 10 * 10 * 10 +
          (v[9] - '0') * 10 * 10 * 10 * 10 * 10 * 10 * 10 * 10 +
          (v[6] - '0') * 10 * 10 * 10 * 10 * 10 * 10 * 10 +
          (v[7] - '0') * 10 * 10 * 10 * 10 * 10 * 10 +
          (v[4] - '0') * 10 * 10 * 10 * 10 * 10 +
          (v[5] - '0') * 10 * 10 * 10 * 10 + (v[2] - '0') * 10 * 10 * 10 +
          (v[3] - '0') * 10 * 10 + (v[0] - '0') * 10 + (v[1] - '0');
  ...
  } else if (t == 0x5) // 32 Bit Real
  {
    ...
    *out = (draw) / scale;
    ...
    warning("(dvparser) Unsupported dif format for extraction to double! "
            "dif=%02x\n",
            dif_vif_key.dif());
    return false;
  }
...
double DVEntry::getCounter(DVEntryCounterType ct) {
  ...
  case DVEntryCounterType::STORAGE_COUNTER:
    return storage_nr.intValue();
  case DVEntryCounterType::TARIFF_COUNTER:
    return tariff_nr.intValue();
  case DVEntryCounterType::SUBUNIT_COUNTER:
    return subunit_nr.intValue();
  case DVEntryCounterType::UNKNOWN:
    break;
  }
...
std::string DVEntry::str() {
  std::string s = tostrprintf(
      "%d: %s %s vif=%x %s%s st=%d ta=%d su=%d", offset,
      dif_vif_key.str().c_str(), toString(measurement_type), vif.intValue(),
      combinable_vifs.size() > 0 ? "HASCOMB " : "",
      combinable_vifs_raw.size() > 0 ? "HASCOMBRAW " : "",
      storage_nr.intValue(), tariff_nr.intValue(), subunit_nr.intValue());
...
bool FieldMatcher::matches(DVEntry &dv_entry) {
  ...
  bool type = (!match_measurement_type ||
               dv_entry.measurement_type == measurement_type);
  bool storage =
      (!match_storage_nr || (dv_entry.storage_nr >= storage_nr_from &&
                             dv_entry.storage_nr <= storage_nr_to));
  bool tariff = (!match_tariff_nr || (dv_entry.tariff_nr >= tariff_nr_from &&
                                      dv_entry.tariff_nr <= tariff_nr_to));
  bool subunit =
      (!match_subunit_nr || (dv_entry.subunit_nr >= subunit_nr_from &&
  ...
  // If field matcher has no combinables, then do NOT match any dventry with a
  // combinable!
  if (vif_combinables.size() == 0 && vif_combinables_raw.size() == 0) {
    // If there is a combinable vif, then there is a raw combinable vif. So
    // comparing both not strictly necessary.
    if (dv_entry.combinable_vifs.size() == 0 &&
        dv_entry.combinable_vifs_raw.size() == 0)
      return true;
    // Oups, field matcher does not expect any combinables, but the dv_entry has
    // combinables. This means no match for us since combinables must be handled
    // explicitly.
    return false;
  }

  // Lets check that the dv_entry vif combinables raw contains the field matcher
  ...
  // Lets check that the dv_entry combinables contains the field matcher
...
std::string FieldMatcher::str() {
  ...
  if (match_storage_nr) {
    s = s + "S(" + std::to_string(storage_nr_from.intValue()) + "-" +
        std::to_string(storage_nr_to.intValue()) + ") ";
  }

  if (match_tariff_nr) {
    s = s + "T(" + std::to_string(tariff_nr_from.intValue()) + "-" +
        std::to_string(tariff_nr_to.intValue()) + ") ";
  }

  if (match_subunit_nr) {
    s += "U(" + std::to_string(subunit_nr_from.intValue()) + "-" +
         std::to_string(subunit_nr_to.intValue()) + ") ";
  }
...
DVEntryCounterType toDVEntryCounterType(const std::string &s) {
  if (s == "storage_counter")
    return DVEntryCounterType::STORAGE_COUNTER;
  if (s == "tariff_counter")
    return DVEntryCounterType::TARIFF_COUNTER;
  if (s == "subunit_counter")
    return DVEntryCounterType::SUBUNIT_COUNTER;
